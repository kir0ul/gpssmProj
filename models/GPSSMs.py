...
class ODGPSSM(GPSSMs):
    ...
    def __init__(
        ...
        self.transition = MultitaskGPModel(
            inducing_points=inducing_points,
            num_tasks=self.state_dim,
            num_latents=self.num_latent_gp,
            MoDep=self.LMC,
            ARD=self.ARD,
        )
        ...
        self.emission_likelihood = MultitaskGaussianLikelihood(
            num_tasks=self.output_dim, has_global_noise=True
        )
    ...
    def forward(self, observations, H=None, input_sequence=None):
        ...
        epsilon = torch.randn(
            torch.Size([self.N_MC, batch_size, self.state_dim]), device=device
        )
        ...
        for t in range(self.seq_len):
            ...
            gp_input = x_t_1.mean(dim=0).expand(
                self.N_MC, batch_size, self.state_dim
            )  # shape: N_MC x batch_size x state_dim
            if input_sequence is not None:
                c_t = input_sequence[:, t].repeat(
                    self.N_MC, 1, 1
                )  # shape: N_MC x batch_size x input_dim
                gp_input = torch.cat(
                    (c_t, x_t_1), dim=-1
                )  # shape: N_MC x batch_size x (input_dim + state_dim)

            if self.LMC:
                tmp = gp_input.repeat(
                    self.num_latent_gp, 1, 1, 1
                )  # shape: num_latent_gp x N_MC x batch_size x (input_dim + state_dim)
                tmp = tmp.transpose(
                    0, 1
                )  # shape: N_MC x num_latent_gp x batch_size x (input_dim + state_dim)
                qf_t = self.transition(
                    tmp
                )  # function distribution: N_MC x batch_size x state_dim
                qx_t = self.likelihood(
                    qf_t
                )  # state distribution: N_MC x batch_size x state_dim
            else:
                tmp = gp_input.repeat(
                    self.state_dim, 1, 1, 1
                )  # shape: state_dim x N_MC x batch_size x state_dim
                tmp = tmp.transpose(
                    0, 1
            ...
            log_ll = (
                log_ll + pyt.log_prob(y_tmp).mean()
            )  # .div(self.seq_len)   # average over particles and batch
    ...
    def prediction(self, observations, H=None, input_sequence=None):
        ...
        for t in range(test_len):
            ...
            if self.LMC:
                tmp = gp_input.repeat(
                    self.num_latent_gp, 1, 1, 1
                )  # shape: num_latent_gp x N_MC x batch_size x state_dim
                tmp = tmp.transpose(
                    0, 1
                )  # shape: N_MC x num_latent_gp x batch_size x state_dim
                qf_t = self.transition(
                    tmp
                )  # function distribution: N_MC x batch_size x state_dim
                qx_t = self.likelihood(
...
class EGPSSM_RNVP(GPSSMs):
    ...
    def forward(self, observations, H=None, input_sequence=None):
        ...
        for t in range(self.seq_len):
            ...
            f_t = f_t.expand(self.state_dim, self.N_MC, batch_size).permute(
                1, 2, 0
            )  # shape: N_MC x batch_size x state_dim
    ...
    def prediction(self, observations, H=None, input_sequence=None):
        ...
        for t in range(test_len):
            ...
            f_t = (
                f_t.mean(dim=0)
                .expand(self.state_dim, self.N_MC, batch_size)
                .permute(1, 2, 0)
            )  # shape: N_MC x batch_size x state_dim
...
class EGPSSM(GPSSMs):
    ...
    def __init__(
        ...
        if self.SAL_flow:
            ...
            G_matrix = Linear_Flow(
                output_dim=self.state_dim,
                NN_parameterized=self.linear_flow_nn_par,
