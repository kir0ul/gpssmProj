...
class Linear_Flow(Flow):
    def __init__(self, output_dim, NN_parameterized=False, input_dim=None) -> None:
        ...
        if NN_parameterized:
            ...
            self.NN_par = Four_Layer_NN(
                input_dim=self.input_dim, output_dim=2 * output_dim, residual=False
            )
    ...
    def forward(
        self, f0: torch.tensor, input_data: torch.tensor = None
    ) -> torch.tensor:
        """
        ...
        if f0.shape[-1] == 1:
            f0 = (
                torch.ones(self.output_dim, device=cg.device, dtype=cg.dtype) * f0
            )  # f0: batch_size x output_dim
        ...
        if self.NN_parameterized:
            ...
            assert input_data.shape[-1] == self.input_dim, (
                "input_data dimension is expected to be [***, ctrl_input_dim + 1]"
            )
            ...
            fk = (
                c * f0 + d
            )  # f0: batch_size x output_dim     fk: batch_size x output_dim
        else:
            # Linear part
            fk = (
                self.c * f0 + self.d
...
class SAL_Flow(Flow):
    ...
    def forward(self, f0: torch.tensor, X: torch.tensor = None) -> torch.tensor:
        ...
        fk_ = torch.sinh(
            b * self.asinh(f0) - a
        )  # f0: [batch_size x 1]     fk_: batch_size x output_dim

        # Linear part
        fk = (
            self.c * fk_ + self.d
        )  # fk_: batch_size x output_dim     fk: batch_size x output_dim
        return fk
...
class SAL_Flows(Flow):
    def __init__(self, num_blocks, output_dim) -> None:
        ...
        self.SAL_blocks_list = [
            SAL_Flow(output_dim=output_dim) for _ in range(num_blocks)
        ]
...
class Tanh_Flows(Flow):
    def __init__(self, num_blocks, output_dim) -> None:
        ...
        self.Tanh_blocks_list = [
            Tanh_Flow(output_dim=output_dim) for _ in range(num_blocks)
        ]
...
class Sinh_ArcsinhFlow(Flow):
    def __init__(
        self, init_a: float, init_b: float, add_init_f0: bool, set_restrictions: bool
    ) -> None:
        """SinhArcsinh Flow
