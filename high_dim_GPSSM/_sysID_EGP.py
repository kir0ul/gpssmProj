    ...
    for jj in range(num_repeat):
        ...
        for epoch in epochs_iter:
            ...
            for i_iter, (U_train, Y_train) in enumerate(train_loader):
                optimizer.zero_grad()
                ELBO, filtered_x, filtered_x_var = model(
                    observations=Y_train.to(cg.device),
            ...
            if epoch % 100 == 0:
                ...
                with torch.no_grad(), gpytorch.settings.fast_pred_var():
                    ...
                    y_pred_1, y_pred_sigma_1 = model.emission(
                    ...
                    y_pred_1, y_pred_sigma_1 = (
                        y_pred_1.view(1, -1, output_dim),
                        y_pred_sigma_1.view(1, -1, output_dim),
                    )
                    ...
                    for i_iter, (U_test, Y_test) in enumerate(test_loader):
                        ...
                        U_test = U_test.view(1, -1, input_dim).to(
                            device
                        )  # shape: 1 x T x input_dim
                        Y_test = Y_test.view(1, -1, output_dim).to(
                            device
                        )  # shape: 1 x T x output_dim
                        ...
                        NLL, pred_x, pred_x_var, pred_mu_2, pred_sigma_2 = (
                            model.Forcasting(
                                T=100, x_0=x, input_sequence=U_test, observations=Y_test
                            )
                        )
                        ...
                        pred_mu = (
                            torch.cat([y_pred_1, pred_mu_2], dim=1)
                            .detach()
                            .cpu()
                            .numpy()
                        )
                        pred_sigma = (
                            torch.cat([y_pred_sigma_1, pred_sigma_2], dim=1)
                            .detach()
                            .cpu()
                            .numpy()
                        )
                ...
                data_sd = (
                    test_loader.dataset.output_normalizer.sd
                ...
                Y = torch.cat(
                    [Y_train.view(1, -1, output_dim), Y_test.detach().cpu()], dim=1
                ).numpy()
                ...
                MSE = np.mean(
                    np.sum(
                        (
                            Y_test_original[:, train_len:, :]
                            - y_pred_mean[:, train_len:, :]
                        )
                        ** 2,
                        axis=-1,
                    )
                ...
                plt.plot(
                    range(T), Y_test_original[0, :, 0], "k-", label="true observations"
                )
                plt.plot(
                    range(T), y_pred_mean[0, :, 0], "b-", label="predicted observations"
                )
                ax.fill_between(
                    range(T),
                    lower[0, :, 0],
                ...
                if save_fig:
                    plt.savefig(
                        DIR + f"prediction_performance_iter{jj}_epoch{epoch}.pdf"
                    )
                plt.show()
                plt.close()
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            for i_iter, (U_test, Y_test) in enumerate(test_loader):
                ...
                # pred_x,          shape: 1 x T x state_dim
                # pred_x_var,      shape: 1 x T x state_dim
                NLL, pred_x, pred_x_var, pred_mu_2, pred_sigma_2 = model.Forcasting(
                    T=100, x_0=x, input_sequence=U_test, observations=Y_test
                )
        ...
        MSE = np.mean(
            np.sum(
                (Y_test_original[:, train_len:, :] - y_pred_mean[:, train_len:, :])
                ** 2,
                axis=-1,
            )
        )
