...
def lorenz96_data_gen(T=6, dt=0.01, D=40, SDE_sigma=0.1, obs_sigma=1.0):
    ...
    for i in range(int(T / dt)):
        # ground truth
        state_target += dt * lorenz96_drift(state_target) + np.sqrt(
            dt
        ) * SDE_sigma * np.random.randn(*state_target.shape)
...
def NS_Kink_function(
    ...
    for i in range(trajectory_length - 1):
        states[i + 1] = nonstationary_kink_func_with_jitter(
            states[i]
        ) + np.random.normal(0.0, process_noise_sd)
        observations[i + 1] = states[i + 1] + np.random.normal(
            0.0, observation_noise_sd
        )
    return states, observations
...
def syn_data_generation(
    ...
    if func == "kinkfunc":
        ...
        true_state_np, obsers_np = NS_Kink_function(
            trajectory_length=traj_len,
            state_int=state_int,
            process_noise_sd=process_noise_sd,
            observation_noise_sd=observation_noise_sd,
...
def plot_1D_all(
    ...
    func="kinkfunc",
    save=False,
    path="./fig_MF1D/2layer_learned_kink_Epoch",
):
    ...
    if model.BNN is not None:
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            test_xx = test_x.reshape(
                -1, 1
            )  # shape: batch_size x (input_dim + state_dim)
            ...
            weights = torch.stack(
                [model.BNN(test_xx) for _ in range(500)]
            )  # weights: 500 x batch_size x (2*state_dim)
            ...
            if model.pureNN:
                ...
                _x_t = (
                    func_pred.unsqueeze(-1) * weights[..., : model.state_dim]
                    + weights[..., model.state_dim :]
                )

            if model.residual_trans:
                # compute the mean and variance of the 100 samples
                pred_val_mean = (
                    _x_t.mean(dim=0) + test_xx
                )  # pred_val_mean shape: batch_size x state_dim
            else:
                # compute the mean and variance of the 100 samples
                pred_val_mean = _x_t.mean(
                    dim=0
                )  # pred_val_mean shape: batch_size x state_dim

            pred_val_var = (
                _x_t.std(dim=0) ** 2 + model.likelihood.noise
            )  # pred_val_var shape: batch_size x state_dim
            ...
            MSE_preGP = (
                1
                / (N_test)
                * np.linalg.norm(pred_val_mean.cpu().numpy().reshape(-1) - y_test) ** 2
            )
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            if model.residual_trans:
                ...
                lower, upper = (
                    ...
                    + test_xx[0, :, :].transpose(-1, -2).mean(dim=[0]),
                    upper.reshape(
                        -1,
                    )
                    + test_xx[0, :, :].transpose(-1, -2).mean(dim=[0]),
