...
for model_name in model_name_list:
    ...
    for D in D_list:
        ...
        t0 = time.time()
        ELBO, X_filter, X_filter_var = model(
...
for i in [2, 6, -1]:
    ax1.text(
        D_list[i],
        parameters_count[0][i],
        f"{parameters_count[0][i]:,}",
        fontsize=16,
        ha="center",
        va="bottom",
        color="b",
    ...
    ax1.text(
        D_list[i],
        parameters_count[1][i],
        f"{parameters_count[1][i]:,}",
        fontsize=16,
        ha="center",
        va="bottom",
        color="r",
...
# 保存图片
plt.savefig(
    "./results/efficiency_comparison/parameters_vs_runtime.pdf",
    dpi=300,
    bbox_inches="tight",
)
# 显示图像
