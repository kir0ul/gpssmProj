...
class EnVI(nn.Module):
            ...
            if self.pureNN:
                self.BNN = nn.Sequential(
                    BayesLinear(
                        prior_mu=0,
                        prior_sigma=0.12,
                        in_features=(dim_c + dim_x),
                        out_features=128,
                    ),
                    nn.ReLU(),
                    BayesLinear(
                        prior_mu=0, prior_sigma=0.12, in_features=128, out_features=64
                    ),
                    nn.ReLU(),
                    BayesLinear(
                        prior_mu=0,
    ...
    def x0_KL(self, observations, input_sequence=None):
        ...
        qm0_KL = torch.distributions.kl.kl_divergence(
            qx0, px0
        ).mean()  # take average over batch_size
    ...
    def forward(self, observations, input_sequence=None, state_init=None):
        ...
        if state_init is None:
            ...
            )  # N_MC x batch_size x state_dim
        else:
            self.RecNet.requires_grad_(False)
            qm0_KL = torch.tensor(
                [0.0], device=observations.device, dtype=observations.dtype
            )
            x_t_1 = state_init.unsqueeze(0).repeat(
                self.N_MC, 1, 1
            )  # N_MC x batch_size x state_dim

        """---------------  2. Calculate likelihood   ---------------"""
        likelihood, filtered_mean, filtered_var, x_t_post_list = self.iterate_sequence(
        ...
        return (
            ...
            filtered_mean,
            filtered_var,
            x_t_post_list.reshape(self.N_MC, -1, self.state_dim),
        )
    ...
    def iterate_sequence(self, observations, x_0, input_sequence=None):
        ...
        if self.consistentSampling:
            ips = (
                self.transition.inducing_points
            ...
            induc_induc_covar = self.transition.kernel(
                self.transition.inducing_points
            ).add_jitter()
        ...
        x_t_post_list = torch.stack(
            x_t_post_list, dim=2
        )  # shape: N_MC x batch_size x seq_len x state_dim
        ...
        if self.pureNN:
            ...
            if self.consistentSampling:
                ...
                GPdynamics = self.transition(
                    gp_input, L
                ).rsample()  # MultivariateNormal, shape: N_MC x batch_size

            _x_t = (
                GPdynamics.unsqueeze(-1) * weights[..., : self.state_dim]
                + weights[..., self.state_dim :]
            )
    ...
    def GP_Predict_forcasting(
        ...
        * GP_Predict 更随机但计算更快，适合用于贝叶斯推断（如粒子滤波等）
        ...
        if self.consistentSampling:
            ...
            GPdynamics = self.transition(
                gp_input, L
            ).mean  # MultivariateNormal, shape: N_MC x batch_size
    ...
    def Forcasting(self, T, x_0, input_sequence=None, observations=None):
        ...
        for i in range(T):
            ...
            # X_mean shape: batch_size x 1 x state_dim
            # P shape: batch_size x state_dim x state_dim
            X, X_mean, P = self.GP_Predict_forcasting(
                x_t_1=x,
                L=L,
                U=U,
                input_sequence=gp_input,
...
class RealNVP_EnVI(nn.Module):
    ...
    def __init__(
        ...
        num_masks = 2
        masks = torch.tensor(
            [np.stack((mask_arr, 1 - mask_arr))] * num_masks, dtype=torch.float32
