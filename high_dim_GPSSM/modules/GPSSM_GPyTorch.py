...
class EnVI(nn.Module):
    ...
    def forward(self, observations, input_sequence=None, state_init=None):
        ...
        if state_init is None:
            ...
            x_t_1 = (
                state_init.unsqueeze(0)
                .unsqueeze(0)
                .repeat(self.N_MC, self.state_dim, 1, 1)
            )
    ...
    def iterate_sequence(self, observations, x_0, input_sequence=None):
        ...
        if self.consistentSampling:
            induc_induc_covar = (
                self.transition.kernel(self.transition.inducing_points)
                .add_jitter()
                .evaluate()
            )
    ...
    def GP_Predict(
        ...
        if self.consistentSampling:
            ...
            GPdynamics = self.transition(
                gp_input, L
            ).rsample()  # MultivariateNormal, shape: N_MC x state_dim x batch_size
    ...
    def GP_Predict_forcasting(
        ...
        if self.consistentSampling:
            ...
            GPdynamics = self.transition(
                gp_input, L
            ).mean  # MultivariateNormal, shape: N_MC x state_dim x batch_size
