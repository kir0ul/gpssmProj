...
def measure_calculate(states, ensembles):
    ...
    average_coverage = (
        torch.sum((states > low_quantile) & (states < high_quantile), dim=1)
        / states.shape[1]
    )
    average_crps = calc_quantile_CRPS(
        states.detach().cpu().numpy(), ensembles.detach().cpu().numpy()
    )
    print("\nPerformance Metrics: ----------------------")
    ...
    return (
        average_rmse.mean().item(),
        average_coverage.mean().item(),
        average_spread.mean().item(),
        average_crps.mean().item(),
    )


def plot_lorenz_trajectory_all(
    states,
