...
def plot_1D_all(
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        ...
        if condition_u:
            ...
            pred_val_mean = observed_pred.mean.mean(dim=[0]) + test_xx[
                0, :, :
            ].transpose(-1, -2).mean(dim=[0])
        ...
        observed_pred_ = gpytorch.distributions.MultivariateNormal(
            pred_val_mean, torch.diag_embed(variance)
        )
    ...
    with torch.no_grad():
        ...
        if func == "ksfunc":
            ...
            raise NotImplementedError(
                "The 'func' input only supports kinkfunc and KSfunc"
            )
...
def plot_1D_NN(
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        ...
        lower = pred_val_mean - 2.0 * model.likelihood.noise.sqrt().view(1, -1)
        upper = pred_val_mean + 2.0 * model.likelihood.noise.sqrt().view(1, -1)
