...
for t in range(T):
    x[t + 1, :] = rslds(x[t, :], A=Atrue, B=btrue, R=Rtrue, r=rtrue, K=K) + np.sqrt(
        varx
    ) * np.random.randn(d_latent)
...
# and the observation
C = (
    npr.rand(d_obs, d_latent + 1) - 0.5
)  # parameters for emission (C = [C, D] for C*x + D
C[:, -1] = np.zeros(d_obs)
...
initial_state = torch.tensor(x[:, 0], dtype=dtype).expand(
    model.N_MC, model.state_dim, 1, model.state_dim
)
maxEpoch = 25
for epoch in range(maxEpoch):
    ...
    for i in dataIter:
        optimizer.zero_grad()
        elbo, x_t, filtered_mean, filtered_var = model(
            x_t_1=x_t_1,
            y_t=torch.tensor(y[:, i][None, None], device=device, dtype=dtype),
        )
    ...
    if epoch == (maxEpoch - 1):
        ...
        cg.save_models(
            model, optimizer, epoch, "empty", DIR, "NASCAR", "_final", save_model=True
        )
