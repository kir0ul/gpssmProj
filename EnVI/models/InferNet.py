...
class Three_Layer_NN(nn.Module):
    def __init__(
        self, input_dim, output_dim, hidden_dim, residual=True, activation="relu"
    ):
        super().__init__()
        if activation == "relu":
            self.activation = torch.relu
...
class Recognition(nn.Module):
    ...
    def __init__(
        self, dim_outputs: int, dim_inputs: int, dim_states: int, length: int
    ) -> None:
        super().__init__()
...
class LSTMRecognition(Recognition):
    ...
    def __init__(
        ...
        self.lstm = nn.LSTM(
            input_size=self.dim_inputs + self.dim_outputs,
        ...
        self.var.bias = nn.Parameter(
            torch.ones(self.dim_states) * torch.tensor(variance), requires_grad=True
        )
...
class MFInference(Recognition):
    ...
    def __init__(
        ...
        self.raw_covar = nn.Linear(
            in_features=in_features, out_features=self.dim_states
        )
    ...
    def forward(self, output_sequence, input_sequence=None):
        ...
        At_all = torch.diag_embed(
            At_all
        )  # At_all shape: batch_size x seq_len x dim_state x dim_state

        raw_cov = safe_softplus(
            self.raw_covar(x)
        )  # raw_cov shape: batch_size x seq_len x dim_state
        Lt_all = torch.diag_embed(
            raw_cov
        )  # raw_cov shape: batch_size x seq_len x dim_state x dim_state
...
class PostNet(nn.Module):
    ...
    def forward(self, x_t_1, hidden):
        ...
        """
        h_tmp = hidden.view(
            hidden.shape[0], 1 + self.bd, -1
        )  # hidden shape: [batch_size x (1+bd) x hidden_size]
        h_combined = h_tmp.sum(1).div(
            2
        )  # h_combined shape: [batch_size x  hidden_size]

        if not self.flag_mf:
            h_combined = (
                1 / 3 * (self.x_to_h(x_t_1) + 2 * h_combined)
            )  # combine the LSTM hidden state with a transformed version of x_t_1
        ...
        epsilon = torch.randn(
            x_t_1.size(), device=x_t_1.device
        )  # sampling x by re-parameterization
