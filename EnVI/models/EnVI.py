...
class GPSSMs(nn.Module):
    def __init__(
        ...
        self.transition = IndependentMultitaskGPModel(
            inducing_points=ips, dim_state=self.state_dim
        )
    ...
    def forward(self, observations, H=None, input_sequence=None):
        ...
        x_t_1 = qx0.rsample(
            torch.Size([self.N_MC, self.state_dim])
        )  # N_MC x state_dim x batch_size x state_dim
        ...
        if self.consistentSampling:
            ...
            U = self.transition(ips).rsample(
                torch.Size([self.N_MC])
        ...
        for t in range(self.seq_len):
            ...
            if self.consistentSampling:
                ...
                GPdynamics_v = self.likelihood(
                    GPdynamics.mean
                )  # MultivariateNormal, shape: N_MC x state_dim x batch_size

            else:
                # 👻 marginalize from U first
                GPdynamics = self.transition(
                    gp_input
                )  # MultivariateNormal, shape: N_MC x state_dim x batch_size
                GPdynamics_v = self.likelihood(
                    GPdynamics
            ...
            x_t = (
                GPdynamics_v.rsample() + x_t_1[:, 0, :, :].transpose(-1, -2).detach()
    ...
    def EnsembleKalmanFilter(
        ...
        if EnKF_likelihood:
            ...
            log_likelihood = d.log_prob(
                y.expand(self.N_MC, -1, -1).transpose(0, 1)
    ...
    def Prediction(self, observations, x_t_1=None, H=None, input_sequence=None):
        ...
        log_ll = torch.tensor(0.0, device=device)  # initialization
        ...
        for t in range(self.seq_len):
            ...
            pyt = self.emission_likelihood(
                yt_mean
            )  # shape:  N_MC x batch_size x output_dim

            y_tmp = observations[:, t].expand(self.N_MC, batch_size, self.output_dim)
            log_ll = log_ll + pyt.log_prob(y_tmp).mean().div(
                test_len
            ...
            x_t_1 = x_t.expand(
                self.N_MC, self.state_dim, self.state_dim, batch_size
            ).permute(0, 1, 3, 2)
        ...
        y_pred = torch.stack(
            y_pred, dim=0
        )  # shape: seq_len x N_MC x batch_size x output_dim
    ...
    def iterate_disc_sequence(self, observations, H=None, input_sequence=None):
        ...
        for t in range(self.seq_len):
            ...
            y_t = observations[:, t]  # shape: batch_size x output_dim
            # x_t_post, shape:  batch_size x N_MC x state_dim
            _, _, _, _, X_mean_post, _ = self.EnsembleKalmanFilter(
                X=x_t,
                y=y_t,
                localization_radius=self.localize_radius,
                linear_obs=True,
...
class OEnVI(nn.Module):
    ...
    def forward(self, observations, x_t_1, H=None, input_sequence=None):
        ...
        for t in range(self.seq_len):
            ...
            x_t = GPdynamics_v.rsample() + x_t_1[:, 0, :, :].transpose(
                -1, -2
            )  # shape: N_MC x state_dim x batch_size
...
class EnVI(nn.Module):
    ...
    def iterate_sequence(self, observations, x_0, input_sequence=None):
        ...
        for t in range(self.seq_len):
            ...
            x_t, X_mean, P = self.GP_Predict(
                x_t_1=x_t_1,
                U=U,
                input_sequence=gp_input,
                localization_radius=self.localize_radius,
            )
            ...
            # update x_t_1 for next round
            x_t_1 = x_t_post.expand(
                self.state_dim, -1, -1, -1
            )  # state_dim x batch_size x N_MC x state_dim
            x_t_1 = x_t_1.permute(
                2, 0, 1, 3
            )  # N_MC x state_dim x batch_size x state_dim
...
class OnlineEnVI(nn.Module):
    ...
    def forward(self, x_t_1, y_t, input_sequence=None, beta=None, verbose=False):
        ...
        likelihood, x_t, filtered_mean, filtered_var = self.iterate_sequence(
            observation=y_t, x_0=x_t_1, input_sequence=input_sequence
        )
    ...
    def GP_Predict(
        ...
        """  ----------------  Update Step  ---------------- """
        noise_Q = torch.diag_embed(
            self.likelihood.noise.view(-1)
        )  # shape: state_dim x state_dim
