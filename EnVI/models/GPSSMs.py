...
def expected_log_prob(
    target: Tensor, q: MultivariateNormal, Covariance: Tensor
) -> Tensor:
    """
...
class vGPSSM(GPSSMs):
    ...
    def forward(self, observations, input_sequence=None):
        ...
        Hx = (
            0.5 * const * math.log(2 * math.pi)
            + torch.logdet(Lt_all).sum(dim=1).mean()
            + const * 0.5
        )
        Hx = Hx.div(self.seq_len)

        """####### ------------  求高斯过程动态转移项 和 数据拟合项  ------------ #########"""
        result_gp_dynamic = torch.tensor(0.0, device=device)
        data_fit = torch.tensor(0.0, device=device)
        ...
        sigma_t_previous = (
            qx0.covariance_matrix
        )  # shape: batch_size x dim_state x dim_state
        ...
        for t in range(self.seq_len):
            ...
            _tmp = (
                At_all[:, t] @ sigma_t_previous @ At_all[:, t].transpose(-1, -2)
            )  # _tmp shape: batch_size x dim_state x dim_state
            sigma_t = _tmp + Lt_all[:, t] @ Lt_all[:, t].transpose(-1, -2)
            # e, v = torch.symeig(sigma_t)
            # print(f"iter: {t}, \n eigenvalue: {e}")
            qxt = MultivariateNormal(
                mt.squeeze(dim=-1), sigma_t
            ).add_jitter()  # shape: batch_size x dim_state
            x_t = qxt.rsample()  # shape:  batch_size x dim_state

            # compute E_q(xt) [log p(yt | xt) ]
            yt = observations[:, t]  # shape: batch_size x dim_output
            emission_noise = torch.diag_embed(
            ...
            data_fit_tmp = qyt.log_prob(yt) - 0.5 * trace_term.diagonal(
                dim1=-2, dim2=-1
            ).sum(dim=-1)
            ...
            if input_sequence is not None:
                ...
                gp_input = torch.cat(
                    (c_t, xt_previous), dim=-1
                )  # shape: batch_size x (dim_input + dim_state)

            # GP dynamics, shape: dim_state x batch_size
            gp_dynamics = self.transition(
                gp_input.expand(
                    self.state_dim, batch_size, (self.state_dim + self.input_dim)
                )
            )
            ...
            _result_gp_dynamic = self.likelihood.expected_log_prob(
                target=_target, input=gp_dynamics
            )
            result_gp_dynamic = result_gp_dynamic + _result_gp_dynamic.mean().div(
                self.seq_len
            )
    ...
    def Prediction(self, observations, input_sequence=None):
        ...
        for t in range(self.seq_len):
            ...
            if self.residual_trans:
                gpMean = (
                    GPdynamics.mean.transpose(-1, -2) + x_t_1.detach()
                )  # shape: batch_size x state_dim
            else:
                gpMean = GPdynamics.mean.transpose(
                    -1, -2
                )  # shape: batch_size x state_dim
            gpVar = GPdynamics.variance.transpose(
                -1, -2
            )  # shape: batch_size x state_dim
            gpCov = torch.diag_embed(
                gpVar, dim1=-2, dim2=-1
            )  # shape: batch_size x state_dim x state_dim

            # shape: batch_size x state_dim
            p = MultivariateNormal(
                gpMean, gpCov + torch.diag_embed(self.likelihood.noise.view(-1))
            )
            # shape: batch_size x output_dim
            pyt = MultivariateNormal(
                p.mean @ self.H.T,
                self.H @ p.covariance_matrix @ self.H.T
...
class VCDT(GPSSMs):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            process_noise_sd=process_noise_sd,
            emission_noise_sd=emission_noise_sd,
            consistentSampling=consistentSampling,
            learn_emission=learn_emission,
            residual_trans=residual_trans,
        )
        ...
        self.inferNet = Four_Layer_NN(
            input_dim=self.output_dim,
            output_dim=output_dim_infNet,
    ...
    def forward(self, observations, input_sequence=None):
        ...
        for t in range(self.seq_len):
            ...
            if self.residual_trans:
                gpMean = (
                    GPdynamics.mean.transpose(-1, -2) + x_t_1[:, 0].detach()
            ...
            parAll = self.inferNet(
                observations[:, t]
            )  # shape: batch_size x (params_dim)
            # construct variational distribution of q(xt | f)
            At = parAll[
                :, 0 : self.state_dim * self.state_dim
            ]  # shape: batch_size x (state_dim x state_dim)
            At = At.view(
            ...
            bt = parAll[
                :,
                self.state_dim * self.state_dim : self.state_dim * (self.state_dim + 1),
            ]  # shape: batch_size x state_dim
            _St = parAll[
            ...
            lower_mask = (
                torch.ones(_St.shape[-2:], device=device)
                .tril(0)
                .expand(batch_size, self.state_dim, self.state_dim)
            )
            St_lower = _St.mul(lower_mask)  # shape: batch_size x state_dim x state_dim
            St = St_lower @ St_lower.transpose(
                -1, -2
            )  # shape: batch_size x state_dim x state_dim

            """-------------------------       obtain the variational distribution      ----------------"""
            # shape: N_MC x batch_size x state_dim x 1
            q_mean = At @ gpMean.unsqueeze(dim=-1) + bt.expand(
            ...
            yt = observations[:, t].expand(
                self.N_MC, batch_size, self.output_dim
            )  # shape: N_MC x batch_size x output_dim
            qyt = MultivariateNormal(
                q_mean @ self.H.T, self.H @ q_cov @ self.H.T
            )  # shape: N_MC x batch_size x output_dim

            likelihood = (
                likelihood
                + self.emission_likelihood.expected_log_prob(target=yt, input=qyt)
                .sum(-1)
                .mean()
            )
