...
def construct_Gaspari_Cohn(loc_radius, x_dim, device):
    def G(z):
        if 0 <= z < 1:
            return (
                1.0 - 5.0 / 3 * z**2 + 5.0 / 8 * z**3 + 1.0 / 2 * z**4 - 1.0 / 4 * z**5
            )
        elif 1 <= z < 2:
            return (
                4.0
...
class AutoEnKF(nn.Module):
    def __init__(
        ...
        self.transition = Four_Layer_NN(
            input_dim=dim_x + dim_c,
            output_dim=dim_x,
        ...
        self.emission_likelihood = gpytorch.likelihoods.GaussianLikelihood(
        ...
        self.RecNet = LSTMRecognition(
            dim_outputs=self.output_dim,
    ...
    def forward(self, observations, input_sequence=None):
        ...
        likelihood, filtered_mean, filtered_var = self.iterate_sequence(
            observations=observations, x_0=x_t_1, input_sequence=input_sequence
        )
    ...
    def iterate_sequence(self, observations, x_0, input_sequence=None):
        ...
        likelihood = torch.tensor(0.0, device=device)
        filtered_mean = []
        filtered_var = []
        ...
        for t in range(self.seq_len):
            ...
            x_t, X_mean, P = self.Prediction(
                x_t_1=x_t_1,
                input_sequence=nn_input,
            ...
            x_t_post, log_likelihood, X_mean_post, P_post = self.EnKFUpdate(
                X=x_t, X_mean=X_mean, P=P, y=y_t
            )
            ...
            filtered_mean.append(
                X_mean_post.squeeze(1)
            )  # shape: batch_size x state_dim
            filtered_var.append(
                P_post.diagonal(offset=0, dim1=-1, dim2=-2)
            )  # shape: batch_size x state_dim

        filtered_mean = torch.stack(
            filtered_mean, dim=1
        )  # shape: batch_size x seq_len x state_dim
        filtered_var = torch.stack(
            filtered_var, dim=1
        )  # shape: batch_size x seq_len x state_dim
    ...
    def emission(self, x, P):
        ...
        """
        pred_mean = self.H @ x
        pred_sigma = self.H @ P @ self.H.transpose(0, 1) + torch.diag_embed(
            self.emission_likelihood.noise.view(-1)
        )
    ...
    def Prediction(
        ...
        if linear_obs and localization_radius is not None:
            taper = construct_Gaspari_Cohn(
                localization_radius, self.state_dim, x_t.device
            )
        else:
            taper = torch.tensor(1.0, device=x_t.device)
        ...
        if var_inflation is not None:
            X = (1.0 + var_inflation) * X_center + X_mean

        P = (
            1 / (self.N_MC - 1) * X_center.transpose(-1, -2) @ X_center
        )  # shape: batch_size x state_dim x state_dim
        P = taper * P
        if var_inflation is not None:
            P = (1.0 + var_inflation) * P

        # add the likelihood noise
        P = P + torch.diag_embed(
            self.likelihood.noise.view(-1)
        )  # shape: batch_size x state_dim x state_dim
    ...
    def EnKFUpdate(self, X, X_mean, P, y=None):
        ...
        """  ----------------  Update Step  ---------------- """
        noise_R = torch.diag_embed(
            self.emission_likelihood.noise.view(-1)
        )  # shape: output_dim x output_dim
        ...
        HPH_T = HP @ self.H.transpose(
            -1, -2
        )  # shape:  batch_size x output_dim x output_dim

        HPH_TR_chol = torch.linalg.cholesky(
            HPH_T + noise_R
        )  # shape: batch_size x output_dim x output_dim, lower-tril

        KalmanGain = (
            torch.cholesky_inverse(HPH_TR_chol) @ HP
        )  # shape:  batch_size x output_dim x state_dim
        ...
        X_mean_post = (
            X_mean + (y.unsqueeze(-2) - HX_m) @ KalmanGain
        )  # shape: batch_size x 1 x state_dim
        ...
        d = torch.distributions.MultivariateNormal(
            HX_m.squeeze(-2), scale_tril=HPH_TR_chol
        )

        log_likelihood = (
            d.log_prob(y).mean().div(self.output_dim)
    ...
    def x0_KL(self, observations, input_sequence=None):
        ...
        px0_cov = torch.diag_embed(
            torch.ones_like(qx0.mean)
        )  # shape: batch_size x state_dim
    ...
    def Forcasting(self, T, x_0, input_sequence=None, observations=None):
        ...
        for i in range(T):
            ...
            # X_mean shape: batch_size x 1 x state_dim
            # P shape: batch_size x state_dim x state_dim
            X, X_mean, P = self.Prediction(
                x_t_1=x,
                input_sequence=nn_input,
                localization_radius=self.localize_radius,
            )
            X_variance = torch.diagonal(
                P, offset=0, dim1=-1, dim2=-2
            )  # shape: batch_size x state_dim

            x = X.permute(1, 0, 2)  # N_MC x batch_size x state_dim

            pred_means = torch.cat(
                [pred_means, X_mean], dim=1
            )  # batch_size x T x state_dim
            pred_sigmas = torch.cat(
                [pred_sigmas, X_variance.unsqueeze(1)], dim=1
            )  # batch_size x T x state_dim

        if observations is not None:
            y_pred, y_pred_sigma = self.emission(
                pred_means.unsqueeze(-1), torch.diag_embed(pred_sigmas)
            )
...
class DKF(nn.Module):
    ...
    def forward(self, observations, input_sequence=None):
        ...
        likelihood, KLD, filtered_mean, filtered_var = self.iterate_sequence(
            observations=observations, x_0=x_t_1, input_sequence=input_sequence
        )
    ...
    def iterate_sequence(self, observations, x_0, input_sequence=None):
        ...
        device = observations.device
        likelihood = torch.tensor(0.0, device=device)
        KL_terms = torch.tensor(0.0, device=device)
        filtered_mean = []
        filtered_var = []
        ...
        hidden_all = self.EncodeNet(
            output_sequence=observations, input_sequence=input_sequence
        )
        ...
        for t in range(self.seq_len):
            ...
            # P_post,           shape: batch_size x state_dim x state_dim
            x_t_post, log_likelihood, KL_terms, X_mean_post, P_post = self.VariUpdate(
                X=x_t, P=PriorDist_xt, H=hidden_all[:, t], y=y_t
            )
    ...
    def Prediction(self, x_t_1, input_sequence=None):
        ...
        """  ----------------  Post-processing step  ---------------- """
        # shape: batch_size x state_dim
        prior_dist_xt = MultivariateNormal(
            mean=x_t, covariance_matrix=torch.diag_embed(self.likelihood.noise.view(-1))
        )
    ...
    def VariUpdate(self, X, P, H, y):
        ...
        Cmt = (self.H @ m_t.unsqueeze(-1)).squeeze(
            dim=-1
        )  # shape:  batch_size x output_dim
        CSigmaC = (
            self.H @ sigma_t @ self.H.T
        )  # shape:  batch_size x output_dim x output_dim
        ...
        data_fit = qyt.log_prob(y) - 0.5 * trace_term.diagonal(dim1=-2, dim2=-1).sum(
            dim=-1
        )
    ...
    def Forcasting(self, T, x_0, input_sequence=None, observations=None):
        ...
        for i in range(T):
            ...
            pred_means = torch.cat(
                [pred_means, x.unsqueeze(1)], dim=1
            )  # batch_size x T x state_dim
            pred_sigmas = torch.cat(
                [pred_sigmas, P.variance.unsqueeze(1)], dim=1
            )  # batch_size x T x state_dim
...
class DSSMs(nn.Module):
    def __init__(
        ...
        self.emission_likelihood = gpytorch.likelihoods.MultitaskGaussianLikelihood(
    ...
    def forward(self, observations, H=None, input_sequence=None):
        ...
        if H is None:
            # emission matrix
            indices = [i for i in range(self.output_dim)]
            H = torch.eye(self.state_dim, device=device, dtype=observations.dtype)[
                indices
            ]
        ...
        for t in range(self.seq_len):
            ...
            NNdynamics = self.transition(
                NN_input
            )  # + x_t_1   # N_MC x batch_size x state_dim

            # shape: N_MC x batch_size x state_dim
            std = torch.sqrt(self.likelihood.noise).expand(
                self.N_MC, batch_size, self.state_dim
            )
            ...
            # x_t_post, shape:  batch_size x N_MC x state_dim
            x_t_post, log_likelihood, X_mean, P, X_mean_post, P_post = (
                self.EnsembleKalmanFilter(
                    X=x_t,
                    y=y_t,
                    localization_radius=self.localize_radius,
                    linear_obs=True,
            ...
            # update x_t_1 for next round
            x_t_1 = x_t_post.transpose(
                0, 1
            ).contiguous()  # N_MC x batch_size x state_dim
    ...
    def EnsembleKalmanFilter(
        ...
        if linear_obs and localization_radius is not None:
            taper = construct_Gaspari_Cohn(
                localization_radius, self.state_dim, X.device
            )
        else:
            taper = torch.tensor(1.0, device=y.device)
        ...
        """  ----------------  Update Step  ---------------- """
        noise_R = torch.diag_embed(
            self.emission_likelihood.task_noises
        )  # shape: output_dim x output_dim
        ...
        )  # shape:  batch_size x N_MC x output_dim

        HP = H @ P  # shape:  batch_size x output_dim x state_dim

        HPH_T = (
            HP @ H.transpose(-1, -2).contiguous()
        )  # shape:  batch_size x output_dim x output_dim

        if torch.__version__ == "1.7.1+cu110":
            HPH_TR_chol = torch.cholesky(
                HPH_T + noise_R
            )  # shape: batch_size x output_dim x output_dim, lower-tril
        else:
            HPH_TR_chol = torch.linalg.cholesky(
        ...
        HX_m = (
            X_mean @ H.transpose(-1, -2).contiguous()
        )  # shape: batch_size x 1 x output_dim
        ...
        if EnKF_likelihood:
            ...
            log_likelihood = d.log_prob(
                y.repeat(self.N_MC, 1, 1).transpose(0, 1)
            )  # shape: batch_size x N_MC
