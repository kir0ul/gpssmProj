...
feedTime = (
    nn_par  # Input-dependent non-stationary EGPSSM. TODO: Auto-Regularization needed
)
...
for ii in range(len(data_name_all)):
    ...
    train_set = dataset(
        ...
        sequence_stride=wd_slides,
        if_time_step=feedTime,
    )
    ...
    if ETGP:
        ...
        if LMC:
            ...
            result_dir = (
                f"./results/{data_name}/LMC_{LMC}_batchSize{batch_size}_seq{seq_len}/"
            )
    ...
    for jj in range(num_repeat):
        ...
        if ETGP:
            ...
            if LMC:
                inducing_points = (
                    4 * torch.rand((num_latent_gp, number_ips, state_dim + input_dim))
                    - 2
                )
            else:
                inducing_points = (
                    4 * torch.rand((state_dim, number_ips, state_dim + input_dim)) - 2
                )
        ...
        for epoch in epochiter:
            ...
            for i_iter, (U_train, Y_train) in enumerate(train_loader):
                ELBO = model(
                    observations=Y_train.to(device), input_sequence=U_train.to(device)
                )
                loss = -ELBO
                optimizer.zero_grad()
                loss.backward()
                ...
                if loss < best_loss:
                    cg.save_best_model(
                        model, optimizer, epoch, result_dir, data_name, jj
                    )
            ...
            if epoch % 200 == 0 or (epoch == start_epoch + num_epoch - 1):
                ...
                with torch.no_grad(), gpytorch.settings.fast_pred_var():
                    for i_iter, (U_test, Y_test) in enumerate(test_loader):
                        y_pred, log_ll = model.prediction(
                            observations=Y_test.to(device),
                ...
                data_sd = torch.tensor(
                    test_loader.dataset.output_normalizer.sd, dtype=dtype
                )  # np array, shape (output_dim, )
                data_mean = torch.tensor(
                    test_loader.dataset.output_normalizer.mean, dtype=dtype
                )
                y_pred_original = (
                    y_pred * data_sd + data_mean
                )  # shape: seq_len x N_MC x batch_size x output_dim
                y_pred_mean = y_pred_original.mean(
                    dim=1
                )  # shape: seq_len x batch_size x output_dim
                y_pred_mean = y_pred_mean.transpose(
                    0, 1
                )  # shape: batch_size x seq_len x output_dim

                Y_test_original = (
                    Y_test.detach().cpu() * data_sd + data_mean
                )  # shape: batch_size x seq_len x output_dim
                MSE = torch.mean(
                    torch.sum((Y_test_original - y_pred_mean) ** 2, dim=-1)
                )
                RMSE = MSE.sqrt()
                print(f"\nRMSE: {RMSE}")
                ...
                # plot the results
                # shape: seq_len x batch_size x output_dim
                y_pred_std = data_sd * y_pred.std(dim=1) * data_sd + torch.sqrt(
                    model.emission_likelihood.task_noises.detach().cpu()
                ).view(-1, output_dim)

                test_len = y_pred_mean.shape[0] * y_pred_mean.shape[1]

                y_test_true = Y_test_original.reshape(
                    test_len,
                )
                ...
                with plt.style.context("ggplot"):
                    ...
                    plt.savefig(
                        result_dir + f"prediction_performance_iter{jj}_epoch{epoch}.png"
                    )
                    # plt.show()
                    plt.close()
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            for i_iter, (U_test, Y_test) in enumerate(test_loader):
                y_pred, log_ll = model.prediction(
                    observations=Y_test.to(device), input_sequence=U_test.to(device)
                )
                y_pred = (
                    y_pred.detach().cpu()
                )  # shape: seq_len x N_MC x batch_size x output_dim
